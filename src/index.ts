...
import { loadConfig, resolveConfig, configPaths, resolveBaseUrl, getWeightedRandom, DEFAULT_SEARCH_MAX, DEFAULT_MAX_PER_PROVIDER } from './config.js';
...
function getPackageVersion(): string {
  // Build-time constant generated by scripts/gen-version.mjs from package.json
  // (run via npm run build / typecheck). Bundled into standalone binaries —
  // never read at runtime from package.json (unreachable inside --compile).
  return VERSION;
}
...
Commands:
  ...
  config [path|check]                            Show config file locations / resolved values
...
Options:
  ...
  --multi            Fan out to all providers (OMNIROUTE_CUSTOM_WEBSEARCH_PROVIDERS or 4 defaults),
  ...
  --config-file <path>   Read configuration from here (default ~/.config/omni-websearch/config)
  --api-key-file <path>  Read the API key from here (default ~/.config/omni-websearch/credentials)

Config (highest precedence first):
  flags             --provider, --max, --config-file, --api-key-file
  environment       OMNIROUTE_CUSTOM_WEBSEARCH_URL, OMNIROUTE_CUSTOM_WEBSEARCH_API_KEY, OMNIROUTE_CUSTOM_WEBSEARCH_PROVIDERS
  config file       ~/.config/omni-websearch/config        URL + providers; never secrets
  credentials file  ~/.config/omni-websearch/credentials   API key, mode 600

Examples:
  omni-websearch search "fp8 quantization" --max 3
  omni-websearch search "attention" --provider exa-search --all-fields
  omni-websearch fetch https://example.com --format markdown
  omni-websearch config check

$XDG_CONFIG_HOME replaces ~/.config. Run "omni-websearch config check" to see
which source each value came from, or "config path" for the files in use.
  `);
...
/** Map the CLI's path flags onto the config loader's inputs. */
function loadOptions(options: ParsedArgs['options']): { configFile?: string; credentialsFile?: string } {
  return { configFile: options.configFile, credentialsFile: options.apiKeyFile };
}

async function runSearch(query: string, options: ParsedArgs['options']): Promise<number> {
  const config = await loadConfig(loadOptions(options));
  ...
  //   OMNIROUTE_CUSTOM_WEBSEARCH_PROVIDERS → fan out over all of them
  ...
  if (options.multi) {
    // ONE call per target. With neither --provider nor OMNIROUTE_CUSTOM_WEBSEARCH_PROVIDERS, the
  ...
  //   OMNIROUTE_CUSTOM_WEBSEARCH_PROVIDERS      → weighted-random pick (first listed = highest weight)
...
async function runHealthcheck(options: ParsedArgs['options']): Promise<void> {
  const config = await loadConfig(loadOptions(options));
  const baseUrl = resolveBaseUrl(config.omniRouteUrl);
  ...
  if (!health.ok) {
    if (health.status === 401 || health.status === 403) {
      throw new Error(`OmniRoute rejected the API key (HTTP ${health.status} ${health.statusText}) — check OMNIROUTE_CUSTOM_WEBSEARCH_API_KEY.`);
    }
    if (health.status > 0) {
...
async function runProviders(options: ParsedArgs['options']): Promise<void> {
  const config = await loadConfig(loadOptions(options));
  const baseUrl = resolveBaseUrl(config.omniRouteUrl);
...
/**
 * `config` — make the resolved configuration, and the source of each value,
 * inspectable. `path` (the default) prints the files in use; `check` resolves
 * them and reports provenance. The key is reported by length, never printed.
 */
async function runConfig(sub: string | undefined, options: ParsedArgs['options']): Promise<number> {
  const paths = configPaths(loadOptions(options));

  if (sub === undefined || sub === 'path') {
    console.log(JSON.stringify(paths, null, 2));
    return 0;
  }
  if (sub !== 'check') {
    throw new Error(`Unknown config subcommand: ${sub}. Expected: path or check.`);
  }

  try {
    const { config, provenance } = resolveConfig(loadOptions(options));
    console.log(JSON.stringify({
      ok: true,
      paths,
      resolved: {
        url: { value: config.omniRouteUrl, source: provenance.url },
        apiKey: { value: `<set, ${config.omniRouteApiKey.length} chars>`, source: provenance.apiKey },
        providers: { value: config.providers ?? null, source: provenance.providers },
        timeout: config.timeout,
      },
    }, null, 2));
    return 0;
  } catch (error) {
    // This subcommand exists to explain configuration, so a resolution failure
    // is reported as data rather than escaping to the generic error handler.
    console.log(JSON.stringify({
      ok: false,
      paths,
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  switch (parsed.command) {
    ...
    case 'fetch':
      ...
      return 0;
    case 'healthcheck':
      await runHealthcheck(parsed.options);
      return 0;
    case 'providers':
      await runProviders(parsed.options);
      return 0;
    case 'config':
      return await runConfig(parsed.query, parsed.options);
    case 'help':
